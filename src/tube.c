...
int rs_tube_catchup(rs_job_t *job) {
    ...
    if (job->copy_len) {
        if (job->stream->eof_in && !job->stream->avail_in && !job->scoop_avail) {
            rs_log(RS_LOG_ERR, "reached end of file while copying literal data through buffers");
 ...
 * output. */
int rs_tube_is_idle(rs_job_t const *job) { return job->write_len == 0 && job->copy_len == 0; }
...
void rs_tube_write(rs_job_t *job, const void *buf, size_t len) {
    ...
    if (len > sizeof(job->write_buf) - job->write_len) {
        rs_fatal("tube popped when trying to write %ld bytes!", (long)len);
    }
