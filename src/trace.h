...
void rs_log0(int level, char const *fn, char const *fmt, ...) __attribute__((format(printf, 3, 4)));

#ifdef DO_RS_TRACE
#define rs_trace(fmt, arg...)                                                                                          \
    do {                                                                                                               \
        rs_log0(RS_LOG_DEBUG, __FUNCTION__, fmt, ##arg);                                                               \
    } while (0)
#else
#define rs_trace(fmt, arg...)
#endif /* !DO_RS_TRACE */

#define rs_log(l, s, str...)                                                                                           \
    do {                                                                                                               \
        rs_log0((l), __FUNCTION__, (s), ##str);                                                                        \
    } while (0)

#define rs_error(s, str...)                                                                                            \
    do {                                                                                                               \
        rs_log0(RS_LOG_ERR, __FUNCTION__, (s), ##str);                                                                 \
    } while (0)

#define rs_fatal(s, str...)                                                                                            \
    do {                                                                                                               \
