...
static rs_result rs_patch_s_cmdbyte(rs_job_t *job) {
    ...
    rs_trace("got command byte 0x%02x (%s), len_1=" PRINTF_FORMAT_U64 "", job->op, rs_op_kind_name(job->cmd->kind),
...
static rs_result rs_patch_s_copy(rs_job_t *job) {
    ...
    rs_trace("COPY(where=" PRINTF_FORMAT_U64 ", len=" PRINTF_FORMAT_U64 ")", PRINTF_CAST_U64(where),
             PRINTF_CAST_U64(len));
...
static rs_result rs_patch_s_copying(rs_job_t *job) {
    ...
    rs_trace("copy " PRINTF_FORMAT_U64 " bytes from basis at offset " PRINTF_FORMAT_U64 "", PRINTF_CAST_U64(len),
